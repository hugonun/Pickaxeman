...
class Pickaxeman():
    ...
    async def download_worker(self, session, log_info, work_deque, download_queue):
        while True:
            try:
    ...
    async def queue_monitor(self, log_info, work_deque, download_results_queue):
    ...
    async def retrieve_certificates(self, url=None, ctl_offset=0, output_directory='/tmp/', concurrency_count=DOWNLOAD_CONCURRENCY):
        async with aiohttp.ClientSession(loop=self.loop, conn_timeout=10) as session:
            ...
            for log in ctl_logs:
                ...
                # Track ctlstart
                if log['url'] in self.ctlstart:
                    self.ctlstart[log['url']]['end'] = log_info['tree_size']
                    ctl_offset = self.ctlstart[log['url']]['start'] - 1
                else:
                    self.ctlstart[log['url']] = {
                        "start": log_info['tree_size'],
                        "end": log_info['tree_size']
                    }
                    continue
                ...
                download_tasks = asyncio.gather(*[
                    self.download_worker(session, log_info, work_deque, download_results_queue)
                    for _ in range(concurrency_count)
                ])

                processing_task    = asyncio.ensure_future(self.processing_coro(download_results_queue, output_dir=output_directory))
                queue_monitor_task = asyncio.ensure_future(self.queue_monitor(log_info, work_deque, download_results_queue))
                ...
                logging.info("Completed {}, stored at {}!".format(
                    ...
                    '{}'.format(log_info['url'].replace('/', '_'))
                ))

                # Update ctlstart
                self.ctlstart[log['url']]['start'] = log_info['tree_size']

                logging.info("Finished downloading and processing {}".format(log_info['url']))


    async def processing_coro(self, download_results_queue, output_dir="/tmp"):
        ...
        while True:
            ...
            if len(entries_iter) > 0:
                workers_response = await process_pool.coro_map(process_worker, entries_iter)
                for wr in workers_response:
                    for r in wr:
    ...
    async def get_certs_and_print(self):
    ...
    def run(self):
        ...
        if args.list_mode:
            self.loop.run_until_complete(self.get_certs_and_print())
            return
        ...
        if args.verbose:
            ...
            logging.basicConfig(format='[%(levelname)s:%(name)s] %(asctime)s - %(message)s', level=logging.WARNING, handlers=handlers)

        logging.info("Starting...")

        if args.ctl_url:
            self.loop.run_until_complete(self.retrieve_certificates(url=args.ctl_url, ctl_offset=int(args.ctl_offset), concurrency_count=args.concurrency_count, output_directory=args.output_dir))
        else:
            self.loop.run_until_complete(self.retrieve_certificates(concurrency_count=args.concurrency_count, output_directory=args.output_dir))
...
def process_worker(result_info):
    ...
    try:
        ...
        for entry in result_info['entries']:
            ...
            if cn_only:
                if cert_data['leaf_cert']['subject']['CN'] is not None:
                    worker_reply.append(cert_data['leaf_cert']['subject']['CN'])
            else:
